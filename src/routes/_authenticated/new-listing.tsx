...
function NewListingPage() {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      // Automatically ensure an inventory item exists
    ...
    onSuccess: (row: any) => {
      if (row.withheld) {
        ...
        toast.success("Listing successfully created", {
          description: "It is live on the browse page and added to your inventory.",
        });
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-6 sm:px-6 sm:py-10">
        <h1 className="font-display text-2xl sm:text-4xl font-black">New listing</h1>
        <p className="mt-1 text-sm text-muted-foreground">Post an item you're willing to swap.</p>
        ...
        <form
          ...
          className="mt-6 space-y-4 rounded-3xl border-2 border-primary/20 bg-card p-4 sm:p-6 shadow-card"
        >
          {(myItems ?? []).length > 0 && (
            <div className="rounded-2xl border-2 border-dashed border-primary/30 bg-primary-soft/40 p-3 sm:p-4">
              ...
              <div className="mt-2 flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                {(myItems ?? []).filter((it) => !listedSet.has(it.id)).map((it) => {
                  ...
                  return (
                    ...
                    >
                      <span className="grid aspect-square place-items-center overflow-hidden rounded-xl bg-primary-soft">
                        {it.image_urls?.[0] ? (
            ...
            <div>
              ...
              {locationChoice === OTHER_LOCATION && (
                <input
                  ...
                  className="mt-2 w-full rounded-full border-2 border-primary/20 bg-white px-4 py-2.5 text-sm outline-none focus:border-primary"
                />
              )}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full rounded-full bg-gradient-primary py-3.5 text-sm font-black uppercase tracking-wider text-primary-foreground shadow-glow disabled:opacity-50 transition hover:scale-[1.01] active:scale-[0.99]"
          >
