  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-[1200px] flex-1 px-6 py-10">
        ...
        {isAdmin && (
          <>
            {/* Modern Categorized Navigation Bar */}
            <div className="mb-8 rounded-3xl border border-border/80 bg-card/60 p-2 sm:p-2.5 backdrop-blur-md shadow-xs">
              <div className="flex flex-col gap-2.5 xl:flex-row xl:items-center">
                {/* 1. Community & Members Group */}
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="hidden sm:inline-block text-[10px] font-black uppercase tracking-wider text-muted-foreground/60 px-2 select-none">
                    Community
                  </span>
                  {[
                    { key: "analytics", label: "Members & Analytics", icon: BarChart3, count: (analytics?.users ?? []).length },
                    { key: "user-message", label: "Message User", icon: Send },
                    ...
                    return (
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-bold transition cursor-pointer select-none active:scale-95 ${
                          isActive
                            ? "bg-gradient-primary text-primary-foreground shadow-sm font-extrabold"
                            : "border border-border/70 bg-background/50 text-muted-foreground hover:text-foreground hover:bg-muted/70 hover:border-border"
                        }`}
                      >
                        ...
                        {item.count !== undefined && (
                          <span
                            className={`ml-0.5 rounded-full px-1.5 py-0.2 text-[10px] font-black tracking-tight ${
                ...
                <div className="hidden xl:block h-6 w-px bg-border/80 mx-1" />

                {/* 2. Moderation & Safety Group */}
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="hidden sm:inline-block text-[10px] font-black uppercase tracking-wider text-muted-foreground/60 px-2 select-none">
                    Moderation
                  </span>
                  {[
                    { key: "flagged", label: "Flagged", icon: Flag, count: (flagged ?? []).length, isAlert: (flagged ?? []).length > 0 },
                    { key: "withheld", label: "Withheld", icon: EyeOff, count: (withheld ?? []).length },
                    { key: "banned", label: "Banned", icon: Ban, count: (banned ?? []).length },
                    { key: "inquiries", label: "Inquiries", icon: LifeBuoy, count: (inquiries ?? []).length },
                  ].map((item) => {
                    const isActive = tab === item.key;
                    const Icon = item.icon;
                    return (
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {[
                    { key: "profile-badges", label: "Profile Badges", icon: Sparkles },
                    { key: "badges", label: "Listing Badges", icon: Award, count: (badges ?? []).length },
                  ].map((item) => {
                    const isActive = tab === item.key;
                    const Icon = item.icon;
                    return (
